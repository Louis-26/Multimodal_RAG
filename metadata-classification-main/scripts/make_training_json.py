...
def parse_args():
    ...
    parser.add_argument(
        '--output_dir',
        type=str,
        default='/home/hltcoe/amartin/SCALE/metadata-classification/data/iter2',
    ...
    for filename in os.listdir(args.metadata_json_dir):
        if filename.endswith('.json'):
            ...
            for metadata_dict in metadata_json:
                ...
                if name not in judgement_dict:
                    print(f'No judgement for {name}')
                    continue
                hf_data_dict["ids"].append(name)
                hf_data_dict["metadata"].append(metadata_string)
                hf_data_dict["video_type"].append(judgement_dict[name])
    
    print(f'Number of videos: {len(hf_data_dict["ids"])}')
